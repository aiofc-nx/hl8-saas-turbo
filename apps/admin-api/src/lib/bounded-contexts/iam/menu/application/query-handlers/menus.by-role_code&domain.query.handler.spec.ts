...
/**
 * MenusByRoleCodeAndDomainQueryQueryHandler 单元测试
 *
 * @description
 * 测试根据角色代码和域查询菜单的查询处理器。
 */
describe('MenusByRoleCodeAndDomainQueryQueryHandler', () => {
  let handler: MenusByRoleCodeAndDomainQueryQueryHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<MenusByRoleCodeAndDomainQueryQueryHandler>(
      MenusByRoleCodeAndDomainQueryQueryHandler,
    );
  ...
  describe('execute', () => {
    ...
    it('应该成功查询菜单列表', async () => {
      ...
      const query = new MenusByRoleCodeAndDomainQuery(roleCode, domain);
