...
/**
 * MenuIdsByRoleIdAndDomainQueryHandler 单元测试
 *
 * @description
 * 测试根据角色ID和域查询菜单ID列表的查询处理器。
 */
describe('MenuIdsByRoleIdAndDomainQueryHandler', () => {
  let handler: MenuIdsByRoleIdAndDomainQueryHandler;
  ...
  beforeEach(async () => {
    ...
    handler = module.get<MenuIdsByRoleIdAndDomainQueryHandler>(
      MenuIdsByRoleIdAndDomainQueryHandler,
    );
  ...
  describe('execute', () => {
    ...
    it('应该成功查询菜单ID列表', async () => {
      ...
      const query = new MenuIdsByRoleIdAndDomainQuery(roleId, domain);

      (repository.findMenusByRoleId as jest.Mock).mockResolvedValue(mockMenus);

      const result = await handler.execute(query);

      expect(result).toEqual([1, 2]);
      expect(repository.findMenusByRoleId).toHaveBeenCalledWith(roleId, domain);
      expect(repository.findMenusByRoleId).toHaveBeenCalledTimes(1);
    });
    ...
    it('应该返回空数组当没有匹配的菜单时', async () => {
      ...
      (repository.findMenusByRoleId as jest.Mock).mockResolvedValue([]);
    ...
    it('应该正确处理查询异常', async () => {
      ...
      (repository.findMenusByRoleId as jest.Mock).mockRejectedValue(error);
