 ...
 * 登录日志模块
 *
 * @description 登录日志功能模块，提供用户登录日志的记录和查询功能。
 * 该模块采用动态注册模式，允许外部注入仓储实现和其他依赖项。
 * 模块包含事件处理器（用于记录登录日志）和查询处理器（用于查询登录日志）。
 *
 * @example
 * ```typescript
 ...
 *       useClass: LoginLogMikroOrmRepository
 *     },
 *     {
...
export class LoginLogModule {
   ...
   * 动态注册登录日志模块
   *
   * @description 动态创建并配置登录日志模块，注册事件处理器、查询处理器和外部依赖。
