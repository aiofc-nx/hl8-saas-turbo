 ...
 * Referrer Policy 类型
 *
 * @description Referrer Policy 的所有可能值
 */
type ReferrerPolicy =
  | 'no-referrer'
 ...
 * 内容安全策略值类型
 *
 * @description CSP 指令的值可以是字符串或字符串数组
 */
 ...
 * Helmet 中间件配置接口
 *
 * @description 定义 Helmet 安全中间件的配置选项
 */
 ...
 * 注册 Helmet 安全中间件
 *
 * @description 为 Fastify 应用注册 Helmet 安全中间件，提供基本的安全防护，包括 XSS、CSP、HSTS 等
 *
 * @param app - Fastify 实例
 * @param config - Helmet 配置（可选，不提供时使用默认配置）
 * @returns Promise<void> 注册成功时返回
 *
 * @example
 * ```typescript
 * // 使用默认配置
