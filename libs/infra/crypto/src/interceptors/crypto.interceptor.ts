 ...
 * HTTP 请求对象接口
 *
 * @description 定义包含 body 和 headers 的请求对象结构
 */
 ...
 * 响应数据类型
 *
 * @description 响应可以是任意类型、字符串或 ApiRes 格式
 */
type ResponseData = unknown | string | ApiRes<unknown>;
...
export class CryptoInterceptor implements NestInterceptor {
  ...
  private decryptRequest(
    ...
    try {
      ...
      // 确保请求体是字符串类型（加密后的数据应该是 Base64 字符串）
      if (typeof request.body !== 'string') {
        this.logger.warn(
          'Request body is not a string, cannot decrypt. Expected encrypted Base64 string.',
        );
        return;
      }
      ...
      this.logger.error(`Failed to decrypt request body: ${errorMessage}`);
      throw error;
    }
  }
  ...
  private encryptResponse(
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : String(error);
      this.logger.error(`Failed to encrypt response: ${errorMessage}`);
      throw error;
    }
  }
