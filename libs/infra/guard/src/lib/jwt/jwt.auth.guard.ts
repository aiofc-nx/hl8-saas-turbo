...
export class JwtAuthGuard extends AuthGuard('jwt') {
   ...
   * 处理认证请求结果
   *
   * @description 当认证失败时抛出未授权异常，成功时返回用户信息。此方法必须匹配基类 `IAuthGuard` 的签名要求。
   *
   * @param err - 认证过程中的错误对象，可能为 Error 实例或 null
   * @param user - 认证成功后的用户对象，泛型类型默认为 IAuthentication
   * @param _info - 认证过程中的额外信息（未使用，为匹配基类签名）
   * @param _context - 执行上下文（未使用，为匹配基类签名）
   * @param _status - 可选的 HTTP 状态码（未使用，为匹配基类签名）
   * @returns 返回用户对象
   *
   * @throws {UnauthorizedException} 当认证失败时抛出
   */
  handleRequest<TUser = IAuthentication>(
    err: Error | null,
