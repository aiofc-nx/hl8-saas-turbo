...
@Module({
  imports: [
    ...
    // API Key 模块(认证：提供API Key 认证策略)
    // 支持简单 API Key 认证和签名请求认证
    // 支持从 Header 或 Query 参数中提取 API Key
    // 支持多种算法：MD5、SHA1、SHA256、HMAC_SHA256
    // 支持事件驱动：验证结果通过事件发射器通知，便于审计和监控
    // 支持分布式：基于 Redis 存储，支持多实例部署
    // 支持灵活配置：支持从配置文件中配置 API Key 认证策略
    // 支持灵活配置：支持从配置文件中配置 API Key 认证策略
    ApiKeyModule,
